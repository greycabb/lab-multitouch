...
class DrawingSurfaceView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defaultStyle: Int = 0) : SurfaceView(context, attrs, defaultStyle), SurfaceHolder.Callback {
    ...
    var ball: Ball //public for easy access
    ...
    fun update() {
        ...
        /* hit detection */
        if (ball.x + ball.radius > viewWidth) { //left bound
            ball.x = viewWidth - ball.radius
            ball.dx *= -1f
        } else if (ball.x - ball.radius < 0) { //right bound
            ball.x = ball.radius
            ball.dx *= -1f
        } else if (ball.y + ball.radius > viewHeight) { //bottom bound
            ball.y = viewHeight - ball.radius
            ball.dy *= -1f
        } else if (ball.y - ball.radius < 0) { //top bound
            ball.y = ball.radius
            ball.dy *= -1f
        }
    }
